...
class ConversationController extends GetxController implements GetxService {
  ...
  void onInit() {
    ...
    channelScrollController.addListener(() {
      if (channelScrollController.position.pixels ==
          channelScrollController.position.maxScrollExtent) {
  ...
  Future<void> getConversation(String channelID, int offset,
      {bool isConversation = true, bool isFromPagination = false}) async {
  ...
  Future<void> sendMessage(String channelID) async {
    ...
    Response response = await conversationRepo.sendMessage(
        conversationController.value.text, channelID, _selectedImageList, "");
    if (response.statusCode == 200) {
